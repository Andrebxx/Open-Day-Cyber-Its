...
// Recupera i livelli dal database (aggiungi eventuale campo file_nome, file_blob, punteggio)
$livelli = [];
...
if ($inizioGioco && $durataGioco) {
    ...
    // Se il tempo è scaduto, aggiorna test_finito e reindirizza
    if (time() > $timestampFine) {
        $stmtFinito = $myDB->prepare("UPDATE Squadra SET test_finito = 1 WHERE nome = ?");
        $stmtFinito->bind_param("s", $teamName);
        $stmtFinito->execute();
        $stmtFinito->close();
...
if ($livelliCompletati >= $totLivelli && $totLivelli > 0) {
    // Aggiorna test_finito PRIMA del redirect
    $stmtFinito = $myDB->prepare("UPDATE Squadra SET test_finito = 1 WHERE nome = ?");
    $stmtFinito->bind_param("s", $teamName);
    $stmtFinito->execute();
    $stmtFinito->close();
